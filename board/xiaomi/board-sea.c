...
LK_FUNC(void, mdelay, 0x4C45E6E8, (uint32_t msec));
...
LK_FUNC(long, partition_read, 0x4C45B400, (const char *part_name, long long offset, uint8_t *data, size_t size));
...
static void cmd_oem_unlock(const char *arg, void *data, unsigned sz) {
    ...
    seccfg_set_lock_state(LKS_UNLOCK);
    mdelay(100);
    mtk_arch_reset(1);
}
...
static void cmd_oem_lock(const char *arg, void *data, unsigned sz) {
    ...
    seccfg_set_lock_state(LKS_DEFAULT);
    mdelay(100);
    mtk_arch_reset(1);
}
...
static void handle_board_info_failure(void) {
    int adc = 0;
    ((void (*)(int, int *))(0x4C418B94 | 1))(2, &adc); // mt_auxadc_read_raw

    if (adc <= 226000) SET_BOARD("miel", "Global", "androidboot.hwversion=1.29.0", "miel");
    else if (adc <= 317001) SET_BOARD("miel",  "India",  "androidboot.hwversion=1.19.0", "miel");
    else if (adc <= 404001) SET_BOARD("fleur", "Global", "androidboot.hwversion=2.39.0", "fleur");
    else if (adc <= 496001) SET_BOARD("miel",  "Global", "androidboot.hwversion=1.49.0", "miel");
    else if (adc <= 585001) SET_BOARD("fleur", "Global", "androidboot.hwversion=2.29.0", "fleurp");
    else if (adc <= 673001) SET_BOARD("miel",  "India",  "androidboot.hwversion=1.19.5", "mielp");
    else return;

    // NOP setting androidboot.hw* to Unknown
    NOP(0x4C427AFA, 2);
    NOP(0x4C427B0C, 2);
    NOP(0x4C427B18, 2);
    NOP(0x4C427B24, 2);
...
void board_early_init(void) {
    ...
    // Xiaomi or Mediatek did a vendor_boot_in_modem for use end of md1img
    // partition as vendor_boot and this is bad for aosp or custom recovery
    // Xiaomi is packaging a working vendor_boot with fastboot ROM but OTA
    // updates is not updating it and vendor_boot may be empty after it so
    // check are vendor_boot partition is valid before NOPing redirection
    char vhdr[8];
    if (partition_read("vendor_boot", 0, (uint8_t *)vhdr, 8) == 8 && memcmp(vhdr, "VNDRBOOT", 8) == 0) {
        NOP(0x4C45FB7E, 1);
        NOP(0x4C459A70, 1);
    }

    // NOP oem unlock and oem lock registration
    NOP(0x4C42AD32, 2);
    NOP(0x4C42AD46, 2);

    // Register our oem_unlock and oem_lock
    fastboot_register("oem unlock", cmd_oem_unlock, 1);
    fastboot_register("oem lock", cmd_oem_lock, 1);
}
