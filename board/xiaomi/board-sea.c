...
#define CMDLINE1_ADDR 0x4C511E4C
#define CMDLINE2_ADDR 0x4C512E50
...
static void spoof_lock_state(void) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF047, 0xFA6F, 0xB350, 0x68FB);
    if (addr) {
        printf("Found sec_usbdl_enabled call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)sec_usbdl_enabled, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF047, 0xFA51, 0xB1A8, 0x68FB);
    if (addr) {
        printf("Found sec_usbdl_enabled call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)sec_usbdl_enabled, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF047, 0xFD00, 0x2800, 0xD1CF);
    if (addr) {
        printf("Found seclib_sec_boot_enabled call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)seclib_sec_boot_enabled, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF047, 0xFCF7, 0x2800, 0xD0E6);
    if (addr) {
        printf("Found seclib_sec_boot_enabled call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)seclib_sec_boot_enabled, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF04A, 0xF928, 0xB1E0, 0x4823);
    if (addr) {
        printf("Found get_unlocked_status call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)get_unlocked_status, TARGET_THUMB);
    }

    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF04A, 0xF93F, 0xB148, 0x482D);
    if (addr) {
        printf("Found get_unlocked_status call at 0x%08X\n", addr);
        PATCH_CALL(addr, (void *)get_unlocked_status, TARGET_THUMB);
    }
    ...
    // uses.
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF04E, 0xFD57, 0x4604, 0xB990);
    if (addr) {
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF005, 0xF808, 0xF001, 0xFC7E);
    if (addr) {
...
void board_early_init(void) {
    printf("Entering early init for Xiaomi Redmi Note 12S\n");
    ...
    addr = SEARCH_PATTERN(LK_START, LK_END, 0xF03A, 0xFAF2, 0x6823, 0x2000);
    if (addr) {
...
void board_late_init(void) {
    printf("Entering late init for Xiaomi Redmi Note 12S\n");
